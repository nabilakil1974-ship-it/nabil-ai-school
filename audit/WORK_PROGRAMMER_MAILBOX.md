...
## OWNER SURVEY EXAMPLES 2 — doctoral AI impact questionnaires (2026-09-19)

Owner supplied multiple MOBILE screenshots of a distinct doctoral example, NOT a reference template to copy. Additional survey domains shown: digital infrastructure/barriers (power outages, internet/subscription prices, devices, language, work conditions), actual AI knowledge/adoption/frequency, task effects, attitudes, future employment security and fear of layoffs/job displacement, and role/contract status. Google Forms GRID screenshot has unusably skinny question-label column, clipped radio choices and horizontal overflow; never reproduce grids as the mobile default. Each statement should be its own mobile-friendly multiple-choice item with five explicit, ordered Likert options. Intro: consent, voluntary/no identifiable personal data if true, eligibility filter for current public education workers. Question formats must be research-specific: one/multiple choice, Likert, frequency, dates, role-specific items, comment box; skip irrelevant role/use branches, do not require AI purposes when nonuser; ensure 'I never use' is exclusive. Explicit metadata per question: axis, question type, choices, required, role/condition, stable numeric code, reverse scale if needed, human-readable analysis label. Critically, CURRENT CODE app/api/routes_research.py build_google_form_script blindly creates addMultipleChoiceItem with five Likert answers FOR EVERY QUESTION regardless of type. This is open product gap, not implemented or live accepted. Need code + tests + Railway build and actual mobile Google Form screenshot to close. Owner examples are examples only; do not impose this domain list on unrelated titles. Preserve real sample demographics, no fabricated survey responses, sources or SPSS runs.


## OWNER PRIORITY LIVE LESSON + EXERCISE + PHONE FIGURE PREVIEW (2026-09-19)

Owner moved research features BELOW education issues. Priority now GitHub issue #6 (Grade 9 math exercise silently fails), #7 physics same, #8 image upload, #9 verified intersection clipped, and PRs #10 exclusive science OCR worker lock, #11 Grade 9 foreign-language lesson scope/TTS, #22 read-only science progress totals. Owner reports lesson words/explanation missing or delayed and third-secondary lessons empty. Wants full lesson/exercise result card + figure preview working on phone and desktop.

PR #25 merged squash `11ae2700ab1c9d68d301efdebf3ac244c3a01de1`, UI build `35459417068` SUCCESS, research build `35459417031` SUCCESS: explicit waiting notice and 85s terminal failure in learning dock, restore pupil text only on FAILED tutor send, avoid double send, ensure nonempty reply cannot disappear if rich renderer returns empty text, visible 'answer arrived / formatting' status, expanded standalone science figure preview with X/backdrop/Escape close, 390px responsive CSS and full-width maths text; third-secondary branch-specific master index is checked BEFORE old unbranched curated index; when branch not selected UI says choose branch. Asset cache-bust version: tutor v17, learning v139, theme v15, strict curriculum v138. CRITICAL DATA GAP: third-secondary branch math master currently has Français (GS 24 / LS 26 / SE 23 / LH 10) but NO English lesson titles; manifest lists English S3 books but catalog TOC cannot be invented. Work must check language/branch-specific empty lists honestly and open indexing gap as needed.

**LIVE RAILWAY ACCEPTANCE NOT YET PROVEN:** capture exact deployed SHA, desktop + 390x844 screenshots/video, Grade 9 Math Pythagoras (6/8 -> 10, area 24), Physics m=2 kg F=10 N -> a=5 m/s², first text appearing and response latency, failed request actionable error with retained input, full lesson derivative/variation readable; third-secondary branch/subject/language lessons loaded from actual list, independent figure card + fullscreen preview, zoom/close Escape X/backdrop preserving labels and colors, no horizontal page scroll, image upload chooser and preview. If reply still silently disappears, inspect underlying HTML inline `sendToAI` route and API traces rather than falsely close #6 or #7. Source of authoritative Grade12 English TOC remains unresolved, do not substitute French titles as English.

**OWNER RESEARCH ACCESS DECISION:** Master's/doctorate should be private admin-only or on separate private site, NOT reachable by students. Must enforce API auth as well as hide button; user has not chosen one of site-vs-admin-protected options, so pending design. No student research deployment claim.


## OWNER ACCEPTANCE SPEC — complete full lesson and open exercise, 2026-09-19 (PRIORITY)

User says finish LESSON + GENERAL EXERCISES on phone (390x844) + desktop together, then return to research. These are owner acceptance requirements, not proof of current completion:

1. Overall UI must use space cleanly, accessible controls, no overlapping input/Home, no giant decorative robot occupying solution, no raw Markdown or system instructions, full responsive long headings/tables and distinct visible visual card.
2. Grade/branch/subject/language combination actually offered must have VERIFIED lessons from relevant official CRDP book; no fabricated index; third-secondary GS/LS/SE/LH audited separately for subjects and languages. CRITICAL: third-secondary math master French counts GS24 LS26 SE23 LH10, English S3 PDF manifests exist but verified English chapter titles absent; do not fake translation and do not report every third-secondary subject as populated.
3. On interactive explanation, first word/next word must reveal as spoken IN the answer card only; prevent extra voice reading text not displayed. Actual synchronized per-word timestamps are needed for exact neural audio alignment; timer interpolation using total duration is NOT sample-accurate, so do not claim accepted from existing prepareSpeechTypewriter; test TTS events/time alignment and fallback for missing TTS.
4. Theoretical/rules/activity/solutions from grade+branch+selected language+book page where available; reject any lesson-grounding claim when book index/page retrieval fails, distinguish from model-created examples.
5. Teacher's voice and explanation should be engaging, kind, student-aged, not a lecture, without switching to unwanted Arabic when English/French book lesson; lesson TITLE language determines main instructional language, with controlled multilingual clarification upon learner request.
6. For FULL LESSON, source-first Activity -> stepwise concepts -> each applicable original figure recreated faithfully with color and dimensions -> red highlighted key rules -> 5 distinct solved exercises -> ALL verified book exercises and problems with exact printed page, original exercise number and subparts a/b/c -> final summary. Do not invent any page, question, source figure, geometry, or claim complete-book coverage when no actual page/book access. Large independent figure preview with multiple lesson figures, color fidelity, close/reopen and optional successive construction steps, including in problem branches.
7. GENERAL EXERCISES independent of fixed subject/grade/language; detect actual question language, optional (prefer) learner grade to constrain methods, accept photos of exercise AND student's handwritten attempted solution, reason from image, mark mistake, correct answer and accurately redrawn diagram, use actual book rules if identifiable, preserve/label unverified sources; full figure preview and projector/full-screen presentation for complete lesson.

Work acceptance script (must capture deployed SHA, actual response latency, screenshot and browser network log on 390x844 AND >=1366 desktop): Grade9 6/8 right triangle solve/draw, Physics F10 m2, image upload and student's incorrect written solution; full Grade9 Lines and Circles EN without secondary analytic-geometry/Arabic preface; grade12 GS and LS mathematics English/French actual lesson coverage vs verified TOC; full lesson chapter activity, figures, red rules, five solved exercises and true book numbered problems; >1 figure large preview preserves color and geometry, X/Escape/backdrop; no voice speaking words outside rendered card and per-word alignment verified against speech timing. For projector test actual fullscreen support instead of assuming responsive sizing means projector mode.

CURRENT PROGRESS: PR #25 merged `11ae2700ab1c9d68d301efdebf3ac244c3a01de1`; UI build `35459417068` SUCCESS, research build `35459417031` SUCCESS, Railway web status success for merge. This is ONLY focused UI/secondary lookup patch, NOT full feature acceptance. No claim that all subjects/grades exist, OCR complete, textbook page alignment/5+all book problems implemented, or per-word TTS synchronized. Work must report missing features as failures and programmer must implement with further cumulative builds. Keep research admin-only before student production exposure; current research APIs not yet access-controlled.


## OWNER NEW FEATURE — BOOK-GROUNDED INTERACTIVE WORKSHEET PER LESSON (2026-09-19)

Owner supplied two VISUAL DESIGN EXAMPLES, **not official textbook pages or sources**: (1) colourful respiratory system grade-six worksheet with 11 phases: Observe -> Predict -> Explore -> Interact -> find evidence -> Think -> Conclude -> Apply -> self-assessment -> reward; labeled body/airflow drawing and CO2/O2 arrows; (2) full circle-perimeter visual lesson sheet, introductory circle/radius sketch, concepts, red/emphasised C=2πr rule, comparison, fully solved radius/diameter examples, visual recap. Ask: integrate a new **Generate interactive worksheet from THIS selected verified lesson** action in NABIL AI, not merely quiz questions. Its source input must be actual CRDP textbook lesson/attached PDF/photo with page refs, grade+branch+subject+lesson+language; do not hallucinate a chapter, figure, page, example, or claim to redraw the book unless the source image exists. If only a topic is known offer clearly labeled illustrative original worksheet rather than claiming it comes from textbook. Adapt phases to age and subject (science observation+evidence; mathematics concept+derivation+worked example+practice; languages reading/vocab/production), never impose respiratory template on other lessons.
