...
(()=>{'use strict';
 ...
 function check(){const selectors=[...document.querySelectorAll('select')];const vals=selectors.map(s=>String(s.value||'')+' '+(s.selectedOptions[0]?.textContent||'' )).join(' ').toLowerCase();const grade=/(التاسع|صف تاسع|grade.?9|eb.?0?9|g.?0?9)/i.test(vals);const physics=/(فيزياء|physics|physique)/i.test(vals);btn.hidden=!(grade&&physics);}
