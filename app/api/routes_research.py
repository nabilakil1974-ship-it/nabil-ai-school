...
def build_google_form_script(request: GoogleFormScriptRequest) -> str:
    ...
    return (
        ...
        "  // Divide long research questionnaires into mobile-friendly axis pages.\n"
        "  form.setProgressBar(true);\n"
        "  let currentAxis = null;\n"
        "  for (const entry of spec.items) {\n"
        "    if (entry.axis !== currentAxis) {\n"
        "      if (currentAxis !== null) {\n"
        "        form.addPageBreakItem().setTitle(entry.axis);\n"
        "      } else {\n"
        "        form.addSectionHeaderItem().setTitle(entry.axis);\n"
        "      }\n"
