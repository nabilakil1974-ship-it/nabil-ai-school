...
SYSTEM_PROMPT = """
SCIENTIFIC_SPEECH_REGISTER_V2 — supersedes previous Arabic-number instructions:
افهم أسئلة الطالب باللهجة اللبنانية، ولكن اجعل جمل الشرح العربية بالفصحى الطبيعية. إذا كانت المادة أو الكتاب بالفرنسية، انطق الأرقام والكميات والوحدات والمصطلحات العلمية بالفرنسية، مثل douze volts و trois cents ohms و zéro virgule zéro quatre ampère؛ وإذا كانت بالإنجليزية، انطقها بالإنجليزية. لا تنطق «اثنا عشر فولت» في شرح كتاب الفيزياء الفرنسي. إذا كان الكتاب عربيًا، اقرأ الأعداد والوحدات بالعربية الفصحى. لا تقرأ slash أو backslash أو شرطة أو الأقواس أو أوامر LaTeX؛ احتفظ بالكسور والجذور والأسس والوحدات مكتوبة بتنسيق علمي، وأنشئ صياغة منطوقة مستقلة عن صياغة اللوح. لا تقرأ الحروف العلمية الأجنبية بتهجئة عربية مصطنعة؛ استخدم نطق لغة المادة. لا تغيّر القيمة العلمية عند إعداد النص الصوتي.
...
محرك حل تمارين الرياضيات لهذا الطلب:
{MATH_EXERCISE_SOLVER_ENGINE}

محركات العلوم في وضع التمارين العامة — اكتشف المادة من السؤال ثم طبّق المحرك الموافق:
{SCIENCE_SUBJECT_ENGINES.get("كيمياء", "")}
{SCIENCE_SUBJECT_ENGINES.get("فيزياء", "")}
{SCIENCE_SUBJECT_ENGINES.get("علوم الحياة", "")}

قاعدة رسم حاسمة:
- إذا ورد في طلب الطالب draw / drawing / diagram / graph / construct / complete the figure / tracer / représenter / schéma / dessiner / ارسم / رسم / مخطط / أكمل الشكل، فالجواب غير مكتمل إطلاقًا من دون DRAWINGS_JSON صالح.
- في واجهة الأستاذ نبيل الحرة: استنتج لغة الجواب من كلام الطالب نفسه. السؤال English يُجاب English، والسؤال Français يُجاب Français، والعربي/اللبناني يُجاب بعربية طبيعية مع إبقاء المصطلحات العلمية الأجنبية عندما تكون جزءًا من السؤال. لا تجعل dropdown الصف/المادة/اللغة شرطًا لهذا الوضع.
- إذا كان الكائن بطبيعته ثلاثي الأبعاد (مجسم هندسي، نموذج جزيئي، جهاز/تركيب يحتاج منظورًا)، استخدم إسقاطًا منظورياً/ثلاثي الأبعاد إذا كان schema الرسم يدعمه، مع إبقاء القياسات والاتجاهات صحيحة. لا تحوّل رسم دالة ثنائي الأبعاد أو مخطط دارة إلى pseudo-3D يغيّر المعنى العلمي.
- لا تقل للطالب إن الرسم موجود قبل أن تنتج DRAWINGS_JSON.
- في الفيزياء يجب أن يمثل الرسم المعطيات الفعلية: topology للدارات، اتجاه القوى والمتجهات، المحور/البؤر والأشعة في البصريات، ومحاور/قيم المنحنى. ممنوع استبدال الرسم المطلوب برسم عام.
- إذا كانت هناك صورة أصلية، حافظ على عناصرها وتسمياتها وعلاقاتها الهندسية/الفيزيائية، ثم أضف فقط ما يطلبه السؤال.
...
"""
        ...
        try:
            ...
            try:
                if _page_request and _page_request[1] == "page":
                    ...
                    # source_chunks from indexed_textbook_page_context() -
                    # a direct, verified database lookup of the EXACT
                    # requested printed page, not a semantic search guess.
                    # That page's own text IS the real exercise content the
                    # student asked for (if it has any) - it must not be
                    # treated as unverified just because it didn't come
                    # through find_nearest_book_exercises(), whose chapter-
                    # end-heading search is designed for a DIFFERENT case
                    # (a general lesson request that needs to look 1-18
                    # pages AHEAD for a "questions and exercises" section,
                    # not "solve exactly what's on page N"). Previously this
                    # branch produced book_exercise_chunks=[] and left every
                    # chunk unmarked, so the prompt's "[VERIFIED BOOK
                    # EXERCISES]" tag never appeared for an exact-page
                    # request - nothing told the model this real page text
                    # WAS the verified source, so it had no strong signal
                    # against filling the gap with plausible invented
                    # exercises when the retrieved text didn't look like a
                    # clean exercise list. Marking the exact page's own
                    # chunks as verified fixes this without ever using
                    # find_nearest_book_exercises' wider, page-drifting
                    # window logic for an exact-page request.
                    book_exercise_chunks = []
                    for _chunk in source_chunks:
                        if isinstance(_chunk, dict):
                            _chunk["is_verified_book_exercise_source"] = True
                else:
        ...
        educational_context = f"""
    محرك المادة العلمية الخاص بهذا الطلب:
    {SCIENCE_SUBJECT_ENGINES.get(_master_subject_key(subject), "غير مطبق.")}

    محرك شرح الرياضيات الخاص بهذا الطلب:
    {MATH_TEACHING_ENGINE if _master_subject_key(subject) == "الرياضيات" else "غير مطبق — استخدم بروتوكول المادة العام."}
    ...
    - في نهاية كل درس كامل فقط، بعد الشرح والرسومات والأمثلة، أخرج بطاقة ختامية بعنوان «📘 خلاصة الدرس» تضم المفاهيم والقواعد الأساسية ومراجع صفحات الكتاب التي تحققت منها. بعدها سطر «🧩 ورقة عمل تفاعلية» دون توليد HTML أو زر داخل النص؛ الواجهة تضيف زرًا حقيقيًا يفتح محرّك ورقة العمل العام لنفس الصف والمادة والدرس. لا تعرض هذه البطاقة عند طلب صفحة منفردة أو تمرين منفرد.
    - ورقة العمل تنشأ ديناميكيًا من صفحات الكتاب المسترجعة وتصحح محاولات الطالب مرحلةً مرحلة؛ لا تكتب تمارين كتاب أو أرقامها إن لم تُسترجع من مصدرها.
...
""".strip()
            ...
            if _page_request is not None:
                ...
                page_route_contract = (
                    ...
                    'CRITICAL - SOLVING "EXERCISES ON PAGE N" MEANS THE REAL '
                    'PRINTED EXERCISES ON THAT EXACT PAGE, NOT A SIMILAR SET '
                    'YOU COMPOSE: the excerpts below marked [VERIFIED BOOK '
                    'EXERCISES] are the actual text extracted from printed '
                    'page {p}. If they contain numbered exercises, solve '
                    'EXACTLY those - same numbers, same given values (e.g. '
                    'if the book says KCl and 12V, your solution uses KCl '
                    'and 12V, never a substituted compound or a rounder '
                    'number), same order, same subparts. Do not compose a '
                    'look-alike exercise on the same topic and present it as '
                    'the book exercise - a student checking against their '
                    'physical book must see the identical problem. If the '
                    'excerpt text for this page contains no clear exercise '
                    'statements (e.g. the OCR text is garbled, or the page '
                    'is mostly a diagram/table the text extraction could not '
                    'capture), say plainly that the exercises on this page '
                    'could not be read reliably from the indexed text and ask '
                    'the student to upload a clear photo of the page instead '
                    'of proceeding to solve an invented substitute.'.replace('{p}', str(_p))
                )
            lesson_prompt = (
                page_route_contract
