...
.teacher .bubble.has-lesson-visual{
    display:grid;grid-template-columns:minmax(300px,34%) minmax(0,1fr);gap:12px;align-items:start;grid-auto-flow:row;
...
/* =========================================================
   FINAL REFERENCE BOARD — built on stable v4 only.
   It visually gathers the previous lesson cards + drawings + quick check
   into one final board, without changing the original lesson/home/avatar flow.
========================================================= */
.lesson-reference-board{
    grid-column:1 / -1 !important;
    width:100% !important;
    ...
    border:2px solid #21e493;
    border-radius:14px;
    background:linear-gradient(180deg,#0a4339,#082f35);
    box-shadow:0 0 20px rgba(33,228,147,.13),inset 0 0 20px rgba(33,228,147,.05);
    color:#eefcf7;
}
.lesson-reference-board-head{
    display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:12px;
}
.lesson-reference-board-check{color:#56f0a9;font-size:22px;font-weight:1000;line-height:1}
.lesson-reference-board-title{color:#56f0a9;font-size:19px;font-weight:900}
.lesson-reference-board-summary{flex:1 1 420px;font-size:14px;line-height:1.55;color:#eaf8f2}
.lesson-reference-board-grid{
    display:grid;
    grid-template-columns:minmax(300px,38%) minmax(0,1fr);
    gap:12px;
    align-items:start;
}
.lesson-reference-board-cards,
.lesson-reference-board-visuals,
.lesson-reference-board-checkarea{
    background:rgba(4,31,43,.55);
    border:1px solid rgba(117,229,255,.25);
    border-radius:12px;
    padding:10px;
    min-width:0;
}
.lesson-reference-board-section-title{
    margin:0 0 9px;
    color:#78e7ff;
    font-weight:900;
    font-size:15px;
}
.lesson-reference-board-cards-inner{display:grid;gap:9px}
.lesson-reference-board .reference-card-copy{
    width:100% !important;
    max-width:100% !important;
    margin:0 !important;
...
.lesson-reference-board .reference-card-copy h2,
.lesson-reference-board .reference-card-copy h3,
.lesson-reference-board .reference-card-copy h4{
    font-size:14px !important;
    margin:4px 0 6px !important;
}
.lesson-reference-board-visuals .lesson-visuals{
    display:grid !important;
    grid-template-columns:repeat(2,minmax(0,1fr));
    gap:9px;
}
.lesson-reference-board-visuals .nabil-visual{margin:0 !important;padding:8px !important}
.lesson-reference-board-visuals .nabil-visual svg{
    width:100% !important;height:auto !important;min-height:210px !important;max-height:310px !important;
}
.lesson-reference-board-checkarea{
    grid-column:1 / -1;
    border-color:rgba(255,212,59,.44);
    background:rgba(55,45,10,.28);
}
.lesson-reference-board-checkarea .lesson-reference-board-section-title{color:#ffe37a}
.lesson-reference-board-checkarea .lesson-check-card{margin:0 !important;width:100% !important;max-width:100% !important}
@media(max-width:900px){
    .lesson-reference-board{padding:10px}
    .lesson-reference-board-grid{grid-template-columns:1fr}
    .lesson-reference-board-checkarea{grid-column:1}
    .lesson-reference-board-visuals .lesson-visuals{grid-template-columns:1fr}
    .lesson-reference-board-summary{flex-basis:100%}
}
...
.drawing-preview-modal{
    position:fixed;inset:0;z-index:12000;background:rgba(0,8,15,.82);display:grid;place-items:center;padding:22px;
}
...
function renderLessonCards(text) {
    ...
    const finalWords = /final answer|final card|conclusion|answer summary|الجواب النهائي|البطاقة النهائية|النتيجة النهائية|الخلاصة النهائية|carte finale|résumé final/i;
    const summaryWords = /rule summary|key idea|summary|خلاصة القاعدة|الفكرة الأساسية|الخلاصة|résumé|idée clé/i;
    const checkWords = /quick check|check question|سؤال تحقق|تحقّق سريع|question de vérification|vérification rapide/i;

    return `<div class="lesson-card-stack">${cards.map((section, index) => {
        const plain = section.replace(/<[^>]+>/g, " ");
        const classes = ["lesson-concept-card", `card-tone-${index % 4 + 1}`];
        if (summaryWords.test(plain)) classes.push("lesson-summary-card");
        if (checkWords.test(plain)) classes.push("lesson-check-card");
        if (finalWords.test(plain) || (cards.length === 1 && summaryWords.test(plain))) classes.push("lesson-final-card");
...
function normalizeFinalLessonCard(bubble, messageDirection, messageLanguage) {
    ...
    // The reference design places the final card below both the concept column
    // and the visual column. Moving it to the bubble makes it span the grid.
    bubble.appendChild(finalCard);
}


function buildFinalReferenceBoard(bubble, messageDirection, messageLanguage) {
    const existing = bubble.querySelector(":scope > .lesson-reference-board");
    if (existing) existing.remove();

    const finalCard = bubble.querySelector(":scope > .lesson-final-card") || bubble.querySelector(".lesson-final-card");
    if (!finalCard) return;

    const allCards = [...bubble.querySelectorAll(".lesson-concept-card")];
    const sourceCards = allCards.filter(card =>
        !card.classList.contains("lesson-final-card") &&
        !card.classList.contains("lesson-check-card")
    );
    const checkCard = allCards.find(card => card.classList.contains("lesson-check-card"));
    const visuals = bubble.querySelector(":scope > .lesson-visuals") || bubble.querySelector(".lesson-visuals");
    ...
    const cardsLabel = messageLanguage === "English"
        ? "Lesson Cards"
        : messageLanguage === "Français"
            ? "Cartes de la leçon"
            : "بطاقات الدرس";

    const visualsLabel = messageLanguage === "English"
        ? "Visual Explanation"
        : messageLanguage === "Français"
            ? "Explication visuelle"
            : "الشرح البصري";

    const checkLabel = messageLanguage === "English"
        ? "Quick Check"
        : messageLanguage === "Français"
            ? "Vérification rapide"
            : "تحقق سريع";

    const summarySource = finalCard.querySelector(".lesson-final-summary") || finalCard;
    const summaryHTML = summarySource.innerHTML || "";

    const board = document.createElement("section");
    board.className = "lesson-reference-board";
    board.setAttribute("dir", messageDirection);

    const head = document.createElement("div");
    head.className = "lesson-reference-board-head";
    head.innerHTML = `
        <span class="lesson-reference-board-check">✓</span>
        <span class="lesson-reference-board-title"></span>
        <div class="lesson-reference-board-summary"></div>
    `;
    head.querySelector(".lesson-reference-board-title").textContent = title;
    head.querySelector(".lesson-reference-board-summary").innerHTML = summaryHTML;

    const grid = document.createElement("div");
    grid.className = "lesson-reference-board-grid";

    const cardsBox = document.createElement("div");
    cardsBox.className = "lesson-reference-board-cards";
    const cardsTitle = document.createElement("div");
    cardsTitle.className = "lesson-reference-board-section-title";
    cardsTitle.textContent = cardsLabel;
    const cardsInner = document.createElement("div");
    cardsInner.className = "lesson-reference-board-cards-inner";
    sourceCards.forEach(card => {
        const copy = card.cloneNode(true);
        copy.classList.add("reference-card-copy");
        cardsInner.appendChild(copy);
    });
    cardsBox.appendChild(cardsTitle);
    cardsBox.appendChild(cardsInner);

    const visualsBox = document.createElement("div");
    visualsBox.className = "lesson-reference-board-visuals";
    const visualsTitle = document.createElement("div");
    visualsTitle.className = "lesson-reference-board-section-title";
    visualsTitle.textContent = visualsLabel;
    visualsBox.appendChild(visualsTitle);
    if (visuals) visualsBox.appendChild(visuals.cloneNode(true));
    else {
        const note = document.createElement("div");
        note.className = "lesson-board-note";
        note.textContent = messageLanguage === "English" ? "No drawing was used in this lesson." : messageLanguage === "Français" ? "Aucun dessin n'a été utilisé dans cette leçon." : "لا يوجد رسم في هذا الدرس.";
        visualsBox.appendChild(note);
    }

    grid.appendChild(cardsBox);
    grid.appendChild(visualsBox);

    const checkBox = document.createElement("div");
    checkBox.className = "lesson-reference-board-checkarea";
    const checkTitle = document.createElement("div");
    checkTitle.className = "lesson-reference-board-section-title";
    checkTitle.textContent = checkLabel;
    checkBox.appendChild(checkTitle);
    if (checkCard) checkBox.appendChild(checkCard.cloneNode(true));
    else {
        const note = document.createElement("div");
        note.className = "lesson-board-note";
        note.textContent = messageLanguage === "English" ? "Continue with the lesson check below." : messageLanguage === "Français" ? "Continuez avec la question de vérification ci-dessous." : "تابع مع سؤال التحقق أدناه.";
        checkBox.appendChild(note);
    }
    ...
    // Replace only the small v4 final bar with the full reference board.
    finalCard.replaceWith(board);
}
...
function layoutLessonBoard(bubble) {
    ...
    const finalCard = bubble.querySelector(":scope > .lesson-final-card") || stack.querySelector(".lesson-final-card");

    const cards = [...stack.querySelectorAll(":scope > .lesson-concept-card")];
    const regularCards = cards.filter(card => !card.classList.contains("lesson-final-card") && !card.classList.contains("lesson-check-card"));
    const checkCards = cards.filter(card => card.classList.contains("lesson-check-card"));

    if (!visuals) {
        const wideStack = document.createElement("div");
        wideStack.className = "lesson-wide-stack";
        regularCards.forEach(card => wideStack.appendChild(card));
        checkCards.forEach(card => wideStack.appendChild(card));
        stack.innerHTML = "";
        stack.appendChild(wideStack);
        return;
    }

    const sideStack = document.createElement("div");
    sideStack.className = "lesson-side-stack";
    const wideStack = document.createElement("div");
    wideStack.className = "lesson-wide-stack";

    const sideCount = Math.min(2, regularCards.length);
    regularCards.slice(0, sideCount).forEach(card => sideStack.appendChild(card));
    regularCards.slice(sideCount).forEach(card => wideStack.appendChild(card));
    checkCards.forEach(card => wideStack.appendChild(card));

    stack.innerHTML = "";
    if (sideStack.children.length) stack.appendChild(sideStack);
    if (visuals.parentElement !== bubble) bubble.appendChild(visuals);
    if (wideStack.children.length) bubble.appendChild(wideStack);
...
function readAnswer(
    ...
    if (
        ...
        alert(
            "ميزة القراءة الصوتية غير مدعومة في هذا المتصفح."
        );
...
function addMessage(
    ...
    if (
        ...
        buildFinalReferenceBoard(bubble, messageDirection, messageLanguage);
...
async function sendToAI(
    ...
    const teachingMode = teachingModeSelect.value || "interactive";
...
اشرح المفهوم الأول فقط بوضوح وبما يناسب الصف، أعط مثالاً قصيراً عند الحاجة، ثم سؤال تحقق واحد فقط.`;
        ...
        addMessage(
            "teacher",
            "حدث خطأ في الاتصال بـ NABIL AI:\n" +
            error.message
        );
...
function nabilFunctionSeries(spec,xmin,xmax,ymin,ymax){
    if(String(spec.type||"").toLowerCase()!=="function" || !spec.function) return [];
    const segments=[];
    let current=[];
    const samples=180;
    for(let i=0;i<=samples;i++){
        ...
        const visible=Number.isFinite(y) && y>=ymin-.25*(ymax-ymin) && y<=ymax+.25*(ymax-ymin);
        if(!visible){
...
function renderNabilDiagram(spec){
    ...
    if (t === "right_triangle" || t === "pythagoras"){
        const a=nabilEsc(L.a||"a"), b=nabilEsc(L.b||"b"), c=nabilEsc(L.c||"c");
        return nabilVisualWrap(title || "مثلث قائم الزاوية",`
        <svg viewBox="0 0 520 320" role="img" aria-label="right triangle">
          <polygon points="95,250 95,70 420,250" class="vfill"/>
          <polyline points="95,220 125,220 125,250" class="vaccent"/>
          <text x="58" y="165" class="vlab">${a}</text>
          <text x="250" y="282" class="vlab">${b}</text>
          <text x="280" y="145" class="vlab">${c}</text>
          <text x="108" y="238" class="vlab vsmall" fill="#dc2626">90°</text>
        </svg>`, spec.note || "");
    }
    ...
    if (t === "triangle"){
        ...
        <svg viewBox="0 0 520 320">
          <polygon points="260,55 80,255 445,255" class="vfill"/>
          <text x="250" y="42" class="vlab">${nabilEsc(L.A||"A")}</text>
          <text x="52" y="276" class="vlab">${nabilEsc(L.B||"B")}</text>
          <text x="454" y="276" class="vlab">${nabilEsc(L.C||"C")}</text>
        </svg>`,spec.note||"");
    }

    if (["square","rectangle","parallelogram","rhombus"].includes(t)){
        let pts = {
            square:"130,70 390,70 390,260 130,260",
            rectangle:"85,90 435,90 435,245 85,245",
            parallelogram:"145,85 430,85 365,250 80,250",
            rhombus:"260,55 430,165 260,275 90,165"
        }[t];
        const names={square:"مربع",rectangle:"مستطيل",parallelogram:"متوازي أضلاع",rhombus:"معيّن"};
        return nabilVisualWrap(title||names[t],`
        <svg viewBox="0 0 520 320">
          <polygon points="${pts}" class="vfill"/>
          <text x="255" y="40" class="vlab">${nabilEsc(L.top||"")}</text>
          <text x="250" y="300" class="vlab">${nabilEsc(L.bottom||"")}</text>
        </svg>`,spec.note||"");
    }
    ...
    if (t === "circle_tangent" || t === "circle"){
        ...
        const radiusLabel=nabilEsc(L.radius||spec.radius||L.r||"r");
        const tangentLabel=nabilEsc(L.tangent_length||L.AM||"AM = ?");
        const hypotenuseLabel=nabilEsc(L.external_distance||L.OM||"");
        return nabilVisualWrap(title || "الدائرة والمماس",`
        <svg viewBox="0 0 520 320">
          <defs><radialGradient id="circleGlow"><stop offset="0" stop-color="#123e5b"/><stop offset="1" stop-color="#061c31"/></radialGradient></defs>
          <circle cx="220" cy="178" r="92" fill="url(#circleGlow)" stroke="#35c8ff" stroke-width="4"/>
          <line x1="220" y1="178" x2="312" y2="178" stroke="#22e58b" stroke-width="4"/>
          <line x1="312" y1="55" x2="312" y2="280" stroke="#ffd43b" stroke-width="4"/>
          <line x1="220" y1="178" x2="312" y2="55" stroke="#e8f6ff" stroke-width="2.5" stroke-dasharray="8 6"/>
          <polyline points="290,178 290,156 312,156" fill="none" stroke="#ff4d4d" stroke-width="3"/>
          <circle cx="220" cy="178" r="6" fill="#35c8ff"/><circle cx="312" cy="178" r="6" fill="#35c8ff"/><circle cx="312" cy="55" r="7" fill="#ff4d4d"/>
          <text x="202" y="202" class="vlab">${center}</text><text x="320" y="196" class="vlab">${tangentPoint}</text><text x="322" y="52" class="vlab">${externalPoint}</text>
          <text x="245" y="168" class="vlab vsmall" style="fill:#22e58b">${radiusLabel}</text>
          <text x="325" y="123" class="vlab vsmall" style="fill:#ffd43b">${tangentLabel}</text>
          ${hypotenuseLabel?`<text x="235" y="105" class="vlab vsmall">${hypotenuseLabel}</text>`:""}
        </svg>`,spec.note||"");
    }
    ...
    if (t === "coordinate_plane" || t === "function" || t === "graph"){
        ...
        series.forEach((line,i)=>{
            ...
            if(poly) curves+=`<polyline points="${poly}" fill="none" stroke="${nabilEsc(line.color||["#22d3ee","#a855f7","#22c55e"][i%3])}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>`;
        });
        ...
        verticals.forEach(v=>{
            const value=Number(typeof v==="object"?v.x:v);
            if(Number.isFinite(value)) curves+=`<line x1="${sx(value)}" y1="45" x2="${sx(value)}" y2="265" stroke="#dc2626" stroke-width="2.5" stroke-dasharray="8 6"/>`;
        });
        ...
        markers.forEach(p=>{
            ...
            if(Number.isFinite(x)&&Number.isFinite(y)) curves+=`<circle cx="${sx(x)}" cy="${sy(y)}" r="5" fill="#169447"/><text x="${sx(x)+7}" y="${sy(y)-7}" class="vlab vsmall">${nabilEsc(p.label||`(${x}, ${y})`)}</text>`;
        });
        const objectPoints=pts.filter(p=>p && !Array.isArray(p) && typeof p==="object");
        objectPoints.forEach((p,i)=>{
            const x=Number(p.x),y=Number(p.y);
            if(!Number.isFinite(x)||!Number.isFinite(y)) return;
            const color=p.color||["#dc2626","#2563eb","#169447","#9333ea"][i%4];
    ...
    if (t === "inclined_plane" || t === "forces"){
        ...
        <svg viewBox="0 0 520 320">
          <polygon points="70,255 440,255 440,85" fill="url(#nabilSurface3d)" stroke="#67e8f9" stroke-width="3" filter="url(#nabilDepth)"/>
          <rect x="280" y="135" width="70" height="55" rx="6" fill="url(#nabilWarm3d)" stroke="#ffd166" stroke-width="3" transform="rotate(-25 315 162)" filter="url(#nabilDepth)"/>
          <line x1="315" y1="160" x2="315" y2="260" class="vaccent"/>
          <polygon points="315,260 306,243 324,243" fill="#dc2626"/>
          <text x="325" y="222" class="vlab" fill="#dc2626">${nabilEsc(L.weight||"mg")}</text>
          <line x1="315" y1="160" x2="270" y2="70" class="vgreen"/>
          <polygon points="270,70 270,90 286,82" fill="#169447"/>
          <text x="230" y="68" class="vlab" fill="#169447">${nabilEsc(L.normal||"N")}</text>
        </svg>`,spec.note||"");
    }

    if (t === "electric_series" || t === "electric_parallel" || t === "electric_mixed" || t === "electric_circuit"){
        if(t==="electric_series"){
            return nabilVisualWrap(title||"دارة كهربائية على التوالي",`
            <svg viewBox="0 0 520 300" role="img" aria-label="series electric circuit">
              <path d="M70 85 H125 M165 85 H185 M225 85 H255 M325 85 H350 M420 85 H455 V235 H70 V85" fill="none" stroke="#e9f8ff" stroke-width="3"/>
              <line x1="125" y1="85" x2="158" y2="67" stroke="#ffd43b" stroke-width="4"/><circle cx="125" cy="85" r="5" fill="#ffd43b"/><circle cx="165" cy="85" r="5" fill="#ffd43b"/>
              <circle cx="205" cy="85" r="20" fill="#0b2941" stroke="#7dd3fc" stroke-width="3"/><text x="197" y="93" class="vlab" style="fill:#7dd3fc">A</text>
              <polyline points="255,85 265,70 278,100 291,70 304,100 317,70 325,85" fill="none" stroke="#f0abfc" stroke-width="4"/>
              <polyline points="350,85 360,70 373,100 386,70 399,100 412,70 420,85" fill="none" stroke="#8bff77" stroke-width="4"/>
              <line x1="255" y1="48" x2="325" y2="48" stroke="#22d3ee" stroke-width="4"/><polygon points="325,48 312,41 312,55" fill="#22d3ee"/>
              <text x="268" y="38" class="vlab vsmall" style="fill:#67e8f9">${nabilEsc(L.current||L.I||"I")}</text>
              <line x1="70" y1="145" x2="70" y2="183" stroke="#fff" stroke-width="3"/><line x1="53" y1="190" x2="87" y2="190" stroke="#ef4444" stroke-width="5"/><line x1="59" y1="207" x2="81" y2="207" stroke="#60a5fa" stroke-width="4"/><line x1="70" y1="207" x2="70" y2="235" stroke="#fff" stroke-width="3"/>
              <text x="91" y="195" class="vlab vsmall" style="fill:#ffd43b">${nabilEsc(L.voltage||L.U||"U")}</text><text x="48" y="177" class="vlab vsmall" style="fill:#ef4444">+</text><text x="49" y="220" class="vlab vsmall" style="fill:#60a5fa">−</text>
              <text x="265" y="126" class="vlab vsmall" style="fill:#f0abfc">${nabilEsc(L.R1||"R₁")}</text><text x="362" y="126" class="vlab vsmall" style="fill:#8bff77">${nabilEsc(L.R2||"R₂")}</text>
            </svg>`,spec.note||"");
        }
        if(t==="electric_parallel"){
            return nabilVisualWrap(title||"دارة كهربائية على التوازي",`
            <svg viewBox="0 0 520 300" role="img" aria-label="parallel electric circuit">
              <path d="M70 75 H130 M170 75 H235 V235 H70 V75 M235 75 H455 V235 H235" fill="none" stroke="#e9f8ff" stroke-width="3"/>
              <line x1="130" y1="75" x2="163" y2="57" stroke="#ffd43b" stroke-width="4"/><circle cx="130" cy="75" r="5" fill="#ffd43b"/><circle cx="170" cy="75" r="5" fill="#ffd43b"/>
              <circle cx="235" cy="125" r="6" fill="#22d3ee"/><circle cx="455" cy="125" r="6" fill="#22d3ee"/>
              <path d="M235 125 H285 L295 110 L308 140 L321 110 L334 140 L347 110 L357 125 H455" fill="none" stroke="#f0abfc" stroke-width="4"/>
              <path d="M235 205 H285 L295 190 L308 220 L321 190 L334 220 L347 190 L357 205 H455" fill="none" stroke="#8bff77" stroke-width="4"/>
              <line x1="275" y1="96" x2="350" y2="96" stroke="#22d3ee" stroke-width="4"/><polygon points="350,96 337,89 337,103" fill="#22d3ee"/>
              <line x1="275" y1="176" x2="350" y2="176" stroke="#38bdf8" stroke-width="4"/><polygon points="350,176 337,169 337,183" fill="#38bdf8"/>
              <text x="291" y="87" class="vlab vsmall" style="fill:#67e8f9">${nabilEsc(L.I1||"I₁")}</text><text x="291" y="167" class="vlab vsmall" style="fill:#67e8f9">${nabilEsc(L.I2||"I₂")}</text>
              <line x1="70" y1="135" x2="70" y2="163" stroke="#fff" stroke-width="3"/><line x1="53" y1="170" x2="87" y2="170" stroke="#ef4444" stroke-width="5"/><line x1="59" y1="187" x2="81" y2="187" stroke="#60a5fa" stroke-width="4"/><line x1="70" y1="187" x2="70" y2="235" stroke="#fff" stroke-width="3"/>
              <text x="91" y="180" class="vlab vsmall" style="fill:#ffd43b">${nabilEsc(L.voltage||L.U||"U")}</text><text x="294" y="151" class="vlab vsmall" style="fill:#f0abfc">${nabilEsc(L.R1||"R₁")}</text><text x="294" y="231" class="vlab vsmall" style="fill:#8bff77">${nabilEsc(L.R2||"R₂")}</text>
            </svg>`,spec.note||"");
        }
        return nabilVisualWrap(title||"دارة كهربائية مختلطة",`
        <svg viewBox="0 0 520 300">
          <path d="M65 65 H190 M250 65 H455 V235 H65 V65" class="vmain"/>
          <line x1="210" y1="43" x2="210" y2="87" class="vmain"/>
          <line x1="228" y1="52" x2="228" y2="78" class="vmain"/>
          <rect x="120" y="219" width="65" height="32" class="vsoft"/>
          <path d="M260 145 H390 M260 205 H390 M260 145 V205 M390 145 V205" class="vmain"/>
          <rect x="292" y="129" width="64" height="32" class="vsoft"/>
          <rect x="292" y="189" width="64" height="32" class="vsoft"/>
        </svg>`,spec.note||"");
    }
    ...
    if (t === "plane"){
        ...
        <svg viewBox="0 0 520 320">
          <polygon points="95,225 185,75 435,105 345,255" fill="url(#nabilPurple3d)" fill-opacity=".72" stroke="#c084fc" stroke-width="3" filter="url(#nabilDepth)"/>
          <line x1="155" y1="205" x2="380" y2="125" class="vaccent"/>
          <circle cx="210" cy="185" r="6" fill="#169447"/>
          <circle cx="315" cy="148" r="6" fill="#169447"/>
    ...
    if (t === "prism"){
        ...
        <svg viewBox="0 0 520 320">
          <polygon points="120,240 210,90 300,240" fill="url(#nabilSurface3d)" stroke="#67e8f9" stroke-width="3" filter="url(#nabilDepth)"/>
          <polygon points="220,210 310,60 400,210" fill="url(#nabilPurple3d)" stroke="#c084fc" stroke-width="3"/>
          <line x1="120" y1="240" x2="220" y2="210" class="vmain"/>
          <line x1="210" y1="90" x2="310" y2="60" class="vmain"/>
          <line x1="300" y1="240" x2="400" y2="210" class="vmain"/>
    ...
    if (t === "pyramid"){
        ...
        <svg viewBox="0 0 520 320">
          <polygon points="120,235 330,235 410,185 205,185" fill="url(#nabilPurple3d)" stroke="#c084fc" stroke-width="3" filter="url(#nabilDepth)"/>
          <line x1="270" y1="55" x2="120" y2="235" class="vmain"/>
          <line x1="270" y1="55" x2="330" y2="235" class="vmain"/>
          <line x1="270" y1="55" x2="410" y2="185" class="vmain"/>
          <line x1="270" y1="55" x2="205" y2="185" stroke="#61758c" stroke-width="2" stroke-dasharray="7 6"/>
          <line x1="270" y1="55" x2="270" y2="210" class="vaccent" stroke-dasharray="7 6"/>
          <text x="280" y="145" class="vlab">${nabilEsc(D.height||L.height||"h")}</text>
        </svg>`,spec.note||"");
    }
    ...
    if (t === "motion"){
        ...
        <svg viewBox="0 0 520 300">
          <line x1="60" y1="225" x2="465" y2="225" class="vmain"/>
          <rect x="145" y="175" width="90" height="50" rx="8" class="vfill"/>
          ...
          <line x1="190" y1="125" x2="325" y2="125" stroke="#dc2626" stroke-width="5"/>
          <polygon points="325,125 307,116 307,134" fill="#dc2626"/>
          <text x="250" y="108" class="vlab" fill="#dc2626">${nabilEsc(L.acceleration||"a")}</text>
        </svg>`,spec.note||"");
    }
    ...
    if (t === "spring"){
        ...
        <svg viewBox="0 0 520 300">
          <line x1="70" y1="70" x2="70" y2="235" class="vmain"/>
          <path d="M70 150 L100 150 L115 125 L145 175 L175 125 L205 175 L235 125 L265 175 L295 150 L330 150" class="vaccent"/>
          <rect x="330" y="115" width="90" height="70" class="vfill"/>
          <text x="355" y="155" class="vlab">${nabilEsc(L.mass||"m")}</text>
    ...
    if (t === "pulley"){
        ...
        <svg viewBox="0 0 520 320">
          <circle cx="260" cy="95" r="55" class="vsoft"/>
          <line x1="205" y1="95" x2="205" y2="255" class="vmain"/>
          <line x1="315" y1="95" x2="315" y2="255" class="vmain"/>
          <rect x="165" y="225" width="80" height="55" class="vfill"/>
          <rect x="275" y="225" width="80" height="55" class="vfill"/>
          <text x="187" y="258" class="vlab">${nabilEsc(L.m1||"m₁")}</text>
          <text x="297" y="258" class="vlab">${nabilEsc(L.m2||"m₂")}</text>
        </svg>`,spec.note||"");
    }
    ...
    if (t === "wave"){
        ...
        <svg viewBox="0 0 520 320">
          <line x1="35" y1="160" x2="485" y2="160" class="vaxis"/>
          <polyline points="${pts}" class="vaccent"/>
          <text x="225" y="90" class="vlab">${nabilEsc(L.amplitude||"A")}</text>
          <text x="320" y="220" class="vlab">${nabilEsc(L.wavelength||"λ")}</text>
        </svg>`,spec.note||"");
    }
    ...
    if (t === "optics_ray"){
        ...
        <svg viewBox="0 0 520 320">
          <line x1="260" y1="45" x2="260" y2="275" class="vaxis" stroke-dasharray="7 6"/>
          <line x1="70" y1="210" x2="260" y2="145" stroke="#2563eb" stroke-width="4"/>
          <line x1="260" y1="145" x2="445" y2="95" stroke="#dc2626" stroke-width="4"/>
          <circle cx="260" cy="145" r="5" fill="#163a67"/>
    ...
    if (t === "probability_tree"){
        ...
        let svg = `<svg viewBox="0 0 760 430" role="img" aria-label="probability tree">
          <circle cx="75" cy="215" r="7" fill="#163a67"/>
          <text x="28" y="202" class="vlab vsmall">${rootLabel}</text>`;
        ...
        first.forEach((branch,i) => {
            ...
            svg += `<line x1="82" y1="215" x2="300" y2="${y}" stroke="#2563eb" stroke-width="4"/>
              <circle cx="300" cy="${y}" r="7" fill="#2563eb"/>
            ...
            children.forEach((child,j) => {
                ...
                svg += `<line x1="307" y1="${y}" x2="540" y2="${cy}" stroke="#169447" stroke-width="4"/>
                  <circle cx="540" cy="${cy}" r="7" fill="#169447"/>
    ...
    if (t === "venn_diagram"){
        ...
        <svg viewBox="0 0 650 390" role="img" aria-label="Venn diagram">
          <rect x="45" y="42" width="560" height="300" rx="18" fill="#f8fbff" stroke="#163a67" stroke-width="3"/>
          <text x="565" y="72" class="vlab vsmall">Ω</text>
          <circle cx="270" cy="190" r="115" fill="#93c5fd" fill-opacity=".62" stroke="#2563eb" stroke-width="4"/>
          <circle cx="390" cy="190" r="115" fill="#86efac" fill-opacity=".58" stroke="#169447" stroke-width="4"/>
    ...
    if (t === "probability_table"){
        ...
        let cells="";
        headers.forEach((h,i)=>{cells+=`<rect x="${x0+i*colW}" y="${y0}" width="${colW}" height="${rowH}" fill="#dbeafe" stroke="#163a67"/><text x="${x0+i*colW+12}" y="${y0+30}" class="vlab vsmall">${nabilEsc(h)}</text>`});
        rows.forEach((row,r)=>{
            const values=Array.isArray(row)?row:Object.values(row||{});
            for(let c=0;c<cols;c++) cells+=`<rect x="${x0+c*colW}" y="${y0+(r+1)*rowH}" width="${colW}" height="${rowH}" fill="#fff" stroke="#6e9dbf"/><text x="${x0+c*colW+12}" y="${y0+(r+1)*rowH+30}" class="vlab vsmall">${nabilEsc(values[c]??"")}</text>`;
        });
    ...
    if (t === "statistics"){
        ...
        vals.forEach((v,i)=>{
            ...
            bars += `<rect x="${x}" y="${250-h}" width="55" height="${h}" fill="${["#2563eb","#169447","#dc2626","#9333ea"][i%4]}" rx="5"/><text x="${x+20}" y="${270}" class="vlab vsmall">${nabilEsc((spec.labels||[])[i]??(i+1))}</text>`;
        });
        return nabilVisualWrap(title||"مخطط إحصائي",`
        <svg viewBox="0 0 520 320">
          <line x1="55" y1="250" x2="480" y2="250" class="vaxis"/>
          <line x1="55" y1="45" x2="55" y2="250" class="vaxis"/>
          ${bars}
        </svg>`,spec.note||"");
    }
    ...
    if (t === "cell_diagram"){
        ...
        <svg viewBox="0 0 520 320">
          ...
          <ellipse cx="335" cy="210" rx="52" ry="27" fill="#38bdf8" fill-opacity=".70" stroke="#67e8f9" stroke-width="3"/>
...
function extractAndRenderNabilDrawings(text){
    ...
    raw = raw.replace(
        ...
        (_,jsonText)=>{
            try{
                ...
                if(rendered) diagrams.push(rendered);
            }catch(err){
                console.warn("NABIL DRAW parse error",err);
            }
            return "";
        }
    );
    ...
    // Hide malformed/incomplete internal drawing payloads instead of printing
    // JSON in the lesson when a provider response is cut off.
...
function speakLessonOpening(){
    ...
    const utterance =
        new SpeechSynthesisUtterance(
            speechText(nabilLessonSpeechText)
        );
    ...
    utterance.onend = () => {
        ...
        lessonStatus(
            "🧠 الأستاذ نبيل يفكر بالفكرة الأولى...",
            true
        );
    };
...
function readCurrentLessonText(){
    ...
    utterance.onend = speechDone;
    utterance.onerror = speechDone;
...
function homeLangCode(){
    ...
    return language === "English" ? "en-US" : language === "Français" ? "fr-FR" : "ar-LB";
}
...
function homeSpeak(text, onDone=null){
    ...
    }catch(e){console.warn("HOME TTS:",e)}
}
...
@media(max-width:520px){
    ...
    .lesson-tutor-message{font-size:10.5px}
        ...
        </head><body><h1>${nabilEsc(title)}</h1><p>${nabilEsc([gradeSelect.value,branchSelect.value,subjectSelect.value,languageSelect.value].filter(Boolean).join(" — "))}</p>${rows}</body></html>`;
    ...
    talkBtn?.addEventListener("click", () => {
        ...
        classicTalkAutoSend = true;
        classicTalkReadReply = true;
